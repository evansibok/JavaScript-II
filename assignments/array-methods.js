...
function isLarge(value) {
  if (value.shirt_size === "L") {
    runnersLargeSizeShirt.push(value);
  }
  return value;
}
runners.filter(isLarge);
...
function donationSum(acc, amount) {
  acc += amount.donation;
  return acc;
}
ticketPriceTotal = runners.reduce(donationSum, 0);
console.log(ticketPriceTotal);

// ticketPriceTotal = runners.reduce((acc, amount) => {
...
// Problem 3
// Return Runners who's company names start with an "S"
...
console.log(runnersWithCompanyNameS);

// runnersWithCompanyNameS = runners.filter((name) => {
//   return name.company_name.charAt(0) === 'S';
// });
// console.log(runnersWithCompanyNameS)
